...
#import --------------------------------------------------------------------------
import re, random, time, argparse, subprocess, os, sys
...
parser.add_argument('-c', '--count', help='numero di mannagge', type=int, default=0, required=False)
...
while True:
    try:
        ...
        continue
    randommannaggia = random.choice(santi).strip()     #check the first word of the random mannaggia and apply article accordingly
    if not randommannaggia:
        continue
    if "madonna" in randommannaggia.split(' ', 1)[0]:
        articolo = " alla"
    elif "papa" in randommannaggia.split(' ', 1)[0]:
                articolo = " al"
    elif "vsf" in args.database:                    #particular case for a db file of aphorisms, imprecazione not needed
        imprecazione = ""
        articolo = ""
    else:
        articolo = " a"
    mannaggia =  imprecazione + articolo + " " + randommannaggia.strip()        #builds the text of the mannaggia
    mannaggia = mannaggia.strip()
    if not mannaggia:
        continue
    print mannaggia
    sys.stdout.flush()
